...
export const projects: Project[] = [
  {
    ...
    longDescription: 'RAGOps Lab is an end-to-end platform that combines agentic Retrieval-Augmented Generation, a built-in evaluation harness, and full observability into a single deployable service. It ingests documents (PDF/text), chunks and embeds them into pgvector with cross-encoder reranking, then answers questions with verifiable citations — refusing when evidence is insufficient. The evaluation engine scores runs across 5 metrics (groundedness, hallucination rate, schema compliance, tool correctness, latency) with 50 test cases and run-vs-run comparison. Every agent step is traced with a custom observability system that tracks duration, token usage, and cost per run. A GitHub Actions CI pipeline fails PRs automatically when eval scores regress. The frontend is a React 19 SPA with Tailwind CSS, featuring real-time SSE streaming for chat responses. Built with FastAPI, OpenAI, pgvector, React, and TypeScript, deployed on Render.',
